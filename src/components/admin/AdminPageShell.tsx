...
const menuItems = [
  { href: "/admin/dashboard", label: "전체 현황" },
  { href: "/admin/workers", label: "근로자" },
  { href: "/admin/companies", label: "건설사" },
  { href: "/admin/sites", label: "현장" },
  { href: "/admin/jobs", label: "일자리" },
  { href: "/admin/applications", label: "지원" },
      ...
      <div className="mx-auto max-w-6xl">
        <header className="rounded-xl border border-blue-100 bg-white p-4 shadow-lg shadow-blue-950/5 sm:p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <nav aria-label="관리자 메뉴" className="grid grid-cols-3 gap-2 sm:flex sm:flex-wrap">
              {menuItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
